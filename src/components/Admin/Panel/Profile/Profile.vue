...
import EditUserModal from '@/components/Admin/Panel/Profile/Modal/EditUserModal.vue'
...
const avatarUrl = computed(() => `https://ui-avatars.com/api/?name=${userStore.user.name}+${userStore.user.last_name}`);

const formattedPhone = computed(() => {
  if (!userStore.user.phone) return ''
  const digits = userStore.user.phone.toString().replace(/\D/g, '') // zostawiamy tylko cyfry
...
<template>
  <div class="p-6 mt-4 md:mt-16 bg-gray-50 min-h-screen">
    <!-- Header profilu -->
    <div class="flex items-center gap-4 mb-6">
      ...
      />
      <div>
        <h2 class="text-2xl font-semibold text-gray-800">{{ userStore.user.name }} {{ userStore.user.last_name }}</h2>
        <p class="text-sm text-gray-500">
          {{ userStore.user.email }}
          <span v-if="userStore.user.email_verified_at" class="ml-2 text-green-600 font-medium">✔ zweryfikowany</span>
        </p>
      </div>
    </div>
      ...
      <div class="bg-white rounded-xl shadow-sm p-6">
        ...
        <div class="space-y-3 text-sm">
          <div class="flex justify-between">
            ...
            <span class="font-medium text-gray-700">{{ userStore.user.name +' '+ userStore.user.last_name }} </span>
          </div>
          <div class="flex justify-between">
    ...
    <!-- Placeholder na przyszłe sekcje -->
    <div
      class="mt-6 h-32 flex items-center justify-center rounded-xl border-2 border-dashed border-gray-300 text-gray-400 bg-white"
    >
      Historia Historii
    </div>
  </div>
